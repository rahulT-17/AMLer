...
class ChainEvaluator(BaseEvaluator) :
    ...
    build_graph`: Converts your flat CSV rows into a map of connections.
       Think of it like building a contact list — "ACC_A called ACC_B and ACC_C". 
    ...
    Recursive call: This is the engine of DFS. For every neighbor of current account, we call `dfs` again with `hops+1`.
      The function calls itself — that's recursion. Each call goes one level deeper until it hits a stop condition.
    ...
    def dfs(self, current, origin, hops, path, graph, found_chains) :
        """
        Depth First Search — follows money hop by hop.
        
        current      — account we're currently visiting
        origin       — account we started from
        hops         — how many hops deep we are
        path         — list of accounts visited so far
        graph        — adjacency list
        found_chains — list we append valid chains into
        """
        # gone deeper than rule allows — stop this path
        if hops > self.rule.max_hops :
            return    # too deep, stop

        if self.rule.detect_cycles : 
            # CYCLE mode — looking for money returning to origin
            if current == origin and hops >= self.rule.min_hops :  # found a cycle back to origin
                found_chains.append(path)
                return 
        else :
            # RANDOM mode — any path long enough counts
    ...
    async def evaluate(self, transactions) -> list :
        ...
        # STEP 3 — if any chains found, create alerts
        for chain in found_chains:
            alerts.append({
                "rule_id": self.rule.rule_id,
                "rule_name": self.rule.name,
                "chain": chain,           # the full path e.g. [A, B, C, A]
